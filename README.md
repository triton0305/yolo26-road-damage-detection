...
AI Hub 데이터로 학습한 모델이 실제 도로에서도 잘 동작하는지 확인하기 위해 강일·미사 일대의 초·중·고교 주변 도로를 Galaxy 스마트폰으로 직접 촬영했습니다.

이 지역은 주거단지와 학교가 가까이 있어 차선, 맨홀, 보수부처럼 여러 도로 요소가 함께 나타나는 생활도로가 많습니다. 그래서 실제 촬영 환경에서 모델의 오탐과 미검출을 확인하고, 추가 학습에 활용하기에 적합하다고 판단했습니다.

수집한 이미지는 Detection 추가 학습 및 평가, Segmentation 도메인 적응, 최종 통합 추론에 활용했습니다.
